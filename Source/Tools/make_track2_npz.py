...
def convert_files_to_npz(input_folder, out_folder, out_prefix):
    ...
    for i in tqdm(range(num)):
        ...
        if ((count >= max_per_train) and (i < num_train)) or (i == num_train - 1):
            ...
            disparity_name = str(out_path) + '/' + out_prefix + \
                '.train.disparity.%1d.npz' % num_files
            left_name = str(out_path) + '/' + out_prefix + '.train.left.%1d.npz' % num_files
            right_name = str(out_path) + '/' + out_prefix + '.train.right.%1d.npz' % num_files
            left_cat_name = str(out_path) + '/' + out_prefix + \
                '.train.left_label.%1d.npz' % num_files
            left_agl_name = str(out_path) + '/' + out_prefix + '.train.left_agl.%1d.npz' % num_files
    ...
    disparity_name = str(out_path) + '/' + out_prefix + '.test.disparity.npz'
    left_name = str(out_path) + '/' + out_prefix + '.test.left.npz'
    right_name = str(out_path) + '/' + out_prefix + '.test.right.npz'
    left_cat_name = str(out_path) + '/' + out_prefix + '.test.left_label.npz'
    left_agl_name = str(out_path) + '/' + out_prefix + '.test.left_agl.npz'
    np.savez_compressed(disparity_name, disparities)
    np.savez_compressed(left_name, lefts)
    np.savez_compressed(right_name, rights)
    np.savez_compressed(left_cat_name, left_categories)
...
if __name__ == '__main__':
    in_folder = '/home2/Documents/DFC2019_track2_trainval/Track_Train/'
    output_folder = '/home2/Documents/DFC2019_track2_trainval/Track_Train_npz_2'
