...
def compute_metrics(test_folder, truth_folder):
    ...
    for i in tqdm(range(num_files)):
        ...
        for cat in range(NUM_CATEGORIES):
            ...
            tp3[cat] += ((test_cls == cat) & (truth_cls == cat) &
                         (truth_cls < NUM_CATEGORIES) & valid_disparity).sum()
            fp3[cat] += ((test_cls == cat) & (truth_cls != cat)
                         & (truth_cls < NUM_CATEGORIES)).sum()
            fn3[cat] += ((test_cls != cat) & (truth_cls == cat)
                         & (truth_cls < NUM_CATEGORIES)).sum()
...
def compute_metrics_v2(cls_folder, dsp_folder, cls_truth, dsp_truth):
    imgNum = cls_truth.shape[0]
    ...
    for i in range(imgNum):
        ...
        test_cls = np.ravel(tifffile.imread(cls_folder + str(i) + 'LEFT_CLS.tif'))
        truth_cls = cls_truth[i]
        test_dsp = np.ravel(tifffile.imread(dsp_folder + str(i) + 'LEFT_DSP.tif'))
        truth_dsp = dsp_truth[i]
...
if __name__ == "__main__":
    ...
    cls_folder = '/home1/jack/Documents/Programs/StereoMatchingNextGen4.0/ResultImg_Val/'
    dsp_folder = '/home1/jack/Documents/Programs/Segmentation/ResultImg_Val/'

    folder = '/home2/Documents/DFC2019_track2_trainval/Track_Train_npz/'
    cls_truth = np.load(folder + 'dfc2019.track2.test.left_label.npz')['arr_0']
    dsp_truth = np.load(folder + 'dfc2019.track2.test.disparity.npz')['arr_0']

    compute_metrics_v2(cls_folder, dsp_folder, cls_truth, dsp_truth)
